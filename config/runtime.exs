import Config

# config/runtime.exs is executed for all environments, including
# during releases. It is executed after compilation and before the
# system starts, so it is typically used to load production configuration
# and secrets from environment variables or elsewhere. Do not define
# any compile-time configuration in here, as it won't be applied.
# The block below contains prod specific runtime configuration.

# Check if we're using Doppler for configuration management
# If DOPPLER_TOKEN is set, rely on DopplerConfigProvider to handle config
# Otherwise, fall back to environment variable configuration
use_doppler = System.get_env("DOPPLER_TOKEN") != nil

if use_doppler do
  # When using Doppler, the custom DopplerConfigProvider will handle
  # fetching and setting all configs at runtime. No additional import needed.
  :ok
else
  # Fall back to traditional environment variable configuration
  # This maintains backward compatibility for existing deployments

  # ## Using releases
  #
  # If you use `mix release`, you need to explicitly enable the server
  # by passing the PHX_SERVER=true when you start it:
  #
  #     PHX_SERVER=true bin/routeros_cm start
  #
  # Alternatively, you can use `mix phx.gen.release` to generate a `bin/server`
  # script that automatically sets the env var above.
  if System.get_env("PHX_SERVER") do
    config :routeros_cm, RouterosCmWeb.Endpoint, server: true
  end

  # Only override port if PORT env var is explicitly set
  if port = System.get_env("PORT") do
    config :routeros_cm, RouterosCmWeb.Endpoint, http: [port: String.to_integer(port)]
  end

  # Mail "from" address configuration
  mail_from_address = System.get_env("MAIL_FROM_ADDRESS", "contact@example.com")
  mail_from_name = System.get_env("MAIL_FROM_NAME", "RouterosCM")
  config :routeros_cm, :mail_from, {mail_from_name, mail_from_address}

  # Mail configuration for containers
  # MAIL_PROVIDER options:
  #   - "local"  : Enable /dev/mailbox preview (for development)
  #   - "logger" : Log emails to console/stdout
  #   - "smtp"   : Use SMTP relay (requires SMTP_* env vars)
  #   - "resend" : Use Resend API (requires RESEND_API_KEY)
  #   - unset    : Use default Swoosh.Adapters.Local
  case System.get_env("MAIL_PROVIDER") do
    "local" ->
      # Enable local mailbox preview at /dev/mailbox
      config :routeros_cm, dev_routes: true
      config :swoosh, local: true
      config :swoosh, :api_client, false
      config :routeros_cm, RouterosCm.Mailer, adapter: Swoosh.Adapters.Local

    "logger" ->
      # Log emails to stdout (useful for containers)
      config :routeros_cm, RouterosCm.Mailer, adapter: Swoosh.Adapters.Logger

    "smtp" ->
      # SMTP relay configuration
      config :routeros_cm, RouterosCm.Mailer,
        adapter: Swoosh.Adapters.SMTP,
        relay: System.get_env("SMTP_HOST", "localhost"),
        port: String.to_integer(System.get_env("SMTP_PORT", "587")),
        username: System.get_env("SMTP_USERNAME"),
        password: System.get_env("SMTP_PASSWORD"),
        ssl: System.get_env("SMTP_SSL", "false") == "true",
        tls: :if_available,
        auth: :if_available

    "resend" ->
      # Resend API configuration
      config :routeros_cm, RouterosCm.Mailer,
        adapter: Swoosh.Adapters.Resend,
        api_key: System.get_env("RESEND_API_KEY") || raise("RESEND_API_KEY is required when MAIL_PROVIDER=resend")

    _ ->
      # Default: keep compile-time configuration
      :ok
  end

  if config_env() == :prod do
    # Use standard PostgreSQL env vars so psql works on the command line
    pghost = System.get_env("PGHOST") || raise "environment variable PGHOST is missing"
    pgdatabase = System.get_env("PGDATABASE") || raise "environment variable PGDATABASE is missing"

    maybe_ipv6 = if System.get_env("ECTO_IPV6") in ~w(true 1), do: [:inet6], else: []

    config :routeros_cm, RouterosCm.Repo,
      hostname: pghost,
      database: pgdatabase,
      username: System.get_env("PGUSER"),
      password: System.get_env("PGPASSWORD"),
      port: String.to_integer(System.get_env("PGPORT") || "5432"),
      pool_size: String.to_integer(System.get_env("POOL_SIZE") || "10"),
      socket_options: maybe_ipv6

    # The secret key base is used to sign/encrypt cookies and other secrets.
    # A default value is used in config/dev.exs and config/test.exs but you
    # want to use a different value for prod and you most likely don't want
    # to check this value into version control, so we use an environment
    # variable instead.
    secret_key_base =
      System.get_env("SECRET_KEY_BASE") ||
        raise """
        environment variable SECRET_KEY_BASE is missing.
        You can generate one by calling: mix phx.gen.secret
        """

    host = System.get_env("PHX_HOST") || "example.com"
    port = String.to_integer(System.get_env("PORT") || "4000")
    # URL_PORT: External port for generated URLs (e.g., 6555 when mapping 6555->4000)
    url_port = String.to_integer(System.get_env("URL_PORT") || System.get_env("PORT") || "4000")
    url_scheme = System.get_env("URL_SCHEME", "https")

    config :routeros_cm, :dns_cluster_query, System.get_env("DNS_CLUSTER_QUERY")

    # Configure credential encryption key
    if credential_key = System.get_env("CREDENTIAL_KEY") do
      config :routeros_cm, :credential_encryption_key, credential_key
    end

    # Build URL config - omit port for standard ports (80/443)
    url_config =
      cond do
        url_scheme == "http" and url_port == 80 -> [host: host, scheme: url_scheme]
        url_scheme == "https" and url_port == 443 -> [host: host, scheme: url_scheme]
        true -> [host: host, port: url_port, scheme: url_scheme]
      end

    config :routeros_cm, RouterosCmWeb.Endpoint,
      url: url_config,
      http: [
        # Enable IPv6 and bind on all interfaces.
        # Set it to  {0, 0, 0, 0, 0, 0, 0, 1} for local network only access.
        # See the documentation on https://hexdocs.pm/bandit/Bandit.html#t:options/0
        # for details about using IPv6 vs IPv4 and loopback vs public addresses.
        ip: {0, 0, 0, 0, 0, 0, 0, 0},
        port: port
      ],
      secret_key_base: secret_key_base

    # ## SSL Support
    #
    # To get SSL working, you will need to add the `https` key
    # to your endpoint configuration:
    #
    #     config :routeros_cm, RouterosCmWeb.Endpoint,
    #       https: [
    #         ...,
    #         port: 443,
    #         cipher_suite: :strong,
    #         keyfile: System.get_env("SOME_APP_SSL_KEY_PATH"),
    #         certfile: System.get_env("SOME_APP_SSL_CERT_PATH")
    #       ]
    #
    # The `cipher_suite` is set to `:strong` to support only the
    # latest and more secure SSL ciphers. This means old browsers
    # and clients may not be supported. You can set it to
    # `:compatible` for wider support.
    #
    # `:keyfile` and `:certfile` expect an absolute path to the key
    # and cert in disk or a relative path inside priv, for example
    # "priv/ssl/server.key". For all supported SSL configuration
    # options, see https://hexdocs.pm/plug/Plug.SSL.html#configure/1
    #
    # We also recommend setting `force_ssl` in your config/prod.exs,
    # ensuring no data is ever sent via http, always redirecting to https:
    #
    #     config :routeros_cm, RouterosCmWeb.Endpoint,
    #       force_ssl: [hsts: true]
    #
    # Check `Plug.SSL` for all available options in `force_ssl`.

    # ## Configuring the mailer
    #
    # In production you need to configure the mailer to use a different adapter.
    # Here is an example configuration for Mailgun:
    #
    #     config :routeros_cm, RouterosCm.Mailer,
    #       adapter: Swoosh.Adapters.Mailgun,
    #       api_key: System.get_env("MAILGUN_API_KEY"),
    #       domain: System.get_env("MAILGUN_DOMAIN")
    #
    # Most non-SMTP adapters require an API client. Swoosh supports Req, Hackney,
    # and Finch out-of-the-box. This configuration is typically done at
    # compile-time in your config/prod.exs:
    #
    #     config :swoosh, :api_client, Swoosh.ApiClient.Req
    #
    # See https://hexdocs.pm/swoosh/Swoosh.html#module-installation for details.
  end

  # End of non-Doppler configuration block
end
