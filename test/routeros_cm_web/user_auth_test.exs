defmodule RouterosCmWeb.UserAuthTest do
  use RouterosCmWeb.ConnCase

  alias Phoenix.LiveView
  alias RouterosCm.Accounts
  alias RouterosCm.Accounts.Scope
  alias RouterosCmWeb.UserAuth

  import RouterosCm.AccountsFixtures

  @remember_me_cookie "_routeros_cm_web_user_remember_me"
  @remember_me_cookie_max_age 60 * 60 * 24 * 14

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, RouterosCmWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      RouterosCmWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :mount_current_scope" do
    ...
    test "assigns scope with nil user if there isn't a valid user_token", %{conn: conn} do
    ...
    test "assigns scope with nil user if there isn't a user_token", %{conn: conn} do
  ...
  describe "on_mount :require_authenticated" do
    ...
    test "redirects to login page if there isn't a valid user_token", %{conn: conn} do
      ...
      socket = %LiveView.Socket{
        endpoint: RouterosCmWeb.Endpoint,
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        RouterosCmWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
